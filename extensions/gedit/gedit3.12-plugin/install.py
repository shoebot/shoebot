...
source_dirs = [here, normpath(join(here, '../../../shoebot/extension'))]
...
def get_dirs_nt(is_admin):
    ...
    if is_admin:
        ...
        dirs = dict(
            dest_dir=dest_dir,
            language_dir=None,  # Not implemented yet
...
def install_plugin(name=None, dest_dir=None, plugin_dir=None, language_dir=None, is_admin=False):
    if is_admin and not isdir(plugin_dir):
        sys.stderr.write('%s not found' % name)
        sys.exit(1)
    else:
        if not is_admin:
            try:
    ...
    source_dir = None
    try:
        copytree(here, plugin_dir)
        for source_dir in source_dirs:
            copytree(source_dir, plugin_dir)
    except Exception as e:
        sys.stderr.write('error attempting to copy %s' % source_dir)
