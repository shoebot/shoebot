# From nodebox-gl
# http://dev.nodebox.net/browser/nodebox-gl/trunk/nodebox/graphics/geometry.py?rev=1071
...
try:
    from nodebox.ext.geometry_math import angle
    from nodebox.ext.geometry_math import coordinates
    from nodebox.ext.geometry_math import distance
except:
    pass
...
def smoothstep(a, b, x):
    """
    Returns a smooth transition between 0.0 and 1.0 using Hermite interpolation
    (cubic spline), where x is a number between a and b.

    The return value will ease (slow down) as x nears a or b. For x smaller than
    a, returns 0.0. For x bigger than b, returns 1.0.
    """
    if x < a:
        return 0.0
    if x >= b:
...
def line_line_intersection(x1, y1, x2, y2, x3, y3, x4, y4, infinite=False):
    """
    Determines the intersection point of two lines, or two finite line segments
    if infinite=False.
...
def point_in_polygon(points, x, y):
    ...
    intersects with the sides of the polygon. If it is an even number of times,
    the point is outside, if odd, inside. The algorithm does not always report
    correctly when the point is very close to the boundary. The polygon is
    passed as a list of (x,y)-tuples.
    """
    odd = False
    n = len(points)
    for i in range(n):
...
class AffineTransform:
    def __init__(self, transform=None):
        """
        A geometric transformation in Euclidean space (i.e. 2D) that preserves
        collinearity and ratio of distance between points.
...
class Bounds:
    def __init__(self, x, y, width, height):
        ...
        The bounding box is an untransformed rectangle that encompasses a shape
        or group of shapes.
        """
    ...
    def contains(self, *a):
        ...
        if len(a) == 1:
            ...
            if isinstance(a, Bounds):
                return (
                    a.x >= self.x
                    and a.x + a.width <= self.x + self.width
                    and a.y >= self.y
                    and a.y + a.height <= self.y + self.height
                )
