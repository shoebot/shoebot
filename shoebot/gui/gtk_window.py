...
from shoebot.core.drawqueue_sink import DrawQueueSink
...
from shoebot.gui.gtk_drawingarea import ShoebotWidget
from shoebot.gui.gtk_input_device import GtkInputDeviceMixin
from shoebot.gui.var_window import VarWindow
...
class ShoebotWindow(Gtk.Window, GtkInputDeviceMixin, DrawQueueSink):
    ...
    def show_variables_window(self):
        ...
        if self.var_window is None and self.bot._vars:
            self.var_window = VarWindow(
                self,
                self.bot,
                "%s variables" % (self.title or "Shoebot"),
            )
    ...
    def trigger_fullscreen_action(self, fullscreen):
        """Toggle fullscreen from outside the GUI, causes the GUI to updated and
        run all its actions."""
    ...
    def main_iteration(self):
        """
        Called from main loop, if your sink needs to handle GUI events do it
        here.
