...
class LSystem(object):
    ...
    def _reset(self):
        ...
        To calculate the number of segments or the total time needed, we need to
        recurse through LSystem._grow(). Before that, the draw(), segments() and
        duration() command will reset both tally variables.
        """
    ...
    def segments(self, generation, time=None):
        ...
        The number of segments that are drawn to the screen depends of the
        number of generations and the amount of time. Each F command has a cost
        that depletes time. Segments will stop being drawn if generation reaches
        0, when there is no time left or when the segment length falls below
        LSystem.threshold.
        """
    ...
    def duration(self, generation):
        ...
        In an animation, the system will expand as time progresses. Each F
        command that draws a segment has a cost that depletes time. To calculate
        the total amount of time for a number of generations, we need to recurse
        through the system. Time does not flow through the system linearly, it
        "branches" from generation to generation.
        """
        _ctx.push()
        self._reset()
