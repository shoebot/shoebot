...
def parse_node(node, paths=[], ignore=["pattern"]):
    ...
    Recures all the children in the node. If a child is something we can draw, a
    line, rect, oval or path, parse it to a PathElement drawable with drawpath()
    """
    ...
    for segment in segments:
        ...
        # Absolute elliptical arc.
            ...
            for i in range(int(len(points) / 7)):
                ...
                for p in arc.elliptical_arc_to(
                    dx,
                    dy,
                    rx,
                    ry,
...
def parse_transform(e, path):
    ...
    Attempts to extract a transform="matrix()|translate()" attribute. Transforms
...
def add_color_info(e, path):
    ...
    Attempts to extract fill and stroke colors from the element and adds it to
    path attributes.
    """
    ...
    try:
        path.fill = _color(get_attribute(e, "fill", default="#00000"), alpha)
    except:
        pass
    try:
        path.stroke = _color(get_attribute(e, "stroke", default="none"), alpha)
    except:
        pass
    try:
        path.strokewidth = float(get_attribute(e, "stroke-width", default="1"))
    except:
        pass
    ...
    path.closed = False
    if path[0].x == path[len(path) - 1].x and path[0].y == path[len(path) - 1].y:
        path.closed = True
    for i in range(1, -1):
