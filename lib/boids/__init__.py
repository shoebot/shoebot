...
class Boid:
    ...
    def cohesion(self, d=100):
        ...
        The centre of mass is the average position of all boids, not including
        itself (the "perceived centre").
        """
    ...
    def limit(self, max=30):
        ...
        Boids can momentarily go very fast, something that is impossible for
        real animals.
        """
...
class Boids(list):
    ...
    def constrain(self):
        ...
        for b in self:
            ...
            if b.y < self.y - dy:
                b.vy += _ctx.random(dy)
            if b.x > self.x + self.w + dx:
                b.vx -= _ctx.random(dx)
            if b.y > self.y + self.h + dy:
                b.vy -= _ctx.random(dy)
            if b.z < 0:
                b.vz += 10
            if b.z > 100:
