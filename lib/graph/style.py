...
class style:
    def __init__(self, name, _ctx, **kwargs):
        ...
        The default style is used for edges. When text is set to None, no id
        label is displayed.
        """
        self.name = name
        self._ctx = _ctx
...
def graph_traffic(s, node, alpha=1.0):
    ...
    s._ctx.nostroke()
    if s.traffic:
        s._ctx.fill(s.traffic.r, s.traffic.g, s.traffic.b, s.traffic.a * alpha)
...
def node(s, node, alpha=1.0):
    ...
    s._ctx.nostroke()
    if s.fill:
        s._ctx.fill(s.fill.r, s.fill.g, s.fill.b, s.fill.a * alpha)
    if s.stroke:
        s._ctx.strokewidth(s.strokewidth)
        s._ctx.stroke(s.stroke.r, s.stroke.g, s.stroke.b, s.stroke.a * alpha * 3)
    r = node.r
    s._ctx.oval(node.x - r, node.y - r, r * 2, r * 2)
...
def node_label(s, node, alpha=1.0):
    ...
    if s.text:
        ...
        s._ctx.nostroke()
        s._ctx.fill(s.text.r, s.text.g, s.text.b, s.text.a * alpha)
...
def edges(s, edges, alpha=1.0, weighted=False, directed=False):
    ...
    if weighted and s.fill:
        ...
        s._ctx.stroke(s.fill.r, s.fill.g, s.fill.b, s.fill.a * 0.65 * alpha)
    ...
    if s.stroke:
        ...
        s._ctx.stroke(s.stroke.r, s.stroke.g, s.stroke.b, s.stroke.a * 0.65 * alpha)
...
def edge(s, path, edge, alpha=1.0):
    ...
    if edge.node2.style == BACK:
        ...
        path.lineto(edge.node2.x, edge.node2.y)
...
def path(s, graph, path):
    ...
    if path and len(path) > 1 and s.stroke:
        ...
        s._ctx.nofill()
        s._ctx.stroke(s.stroke.r, s.stroke.g, s.stroke.b, s.stroke.a)
        if s.name != DEFAULT:
            s._ctx.strokewidth(s.strokewidth)
        else:
