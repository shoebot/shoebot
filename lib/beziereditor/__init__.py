...
from nodebox.graphics import CLOSE
from nodebox.graphics import CURVETO
from nodebox.graphics import LINETO
from nodebox.graphics import MOVETO
from nodebox.graphics import PathElement
...
class MagneticGrid:
    ...
    def snap(self, x, y, treshold=0.4):
        ...
        if x - l < treshold and y - t < treshold:
            gx, gy = l, t  # snap top left
        if r - x < treshold and y - t < treshold:
            gx, gy = r, t  # snap top right
        if r - x < treshold and b - y < treshold:
            gx, gy = r, b  # snap bottom right
        if x - l < treshold and b - y < treshold:
            gx, gy = l, b  # snap bottom left
...
class BezierPathEditor:
    ...
    def overlap(self, x1, y1, x2, y2, r=5):
        ...
        There is an r treshold in which point 1 and point 2 are considered to
        overlap.
        """
    ...
    def contains_point(self, x, y, d=2):
        ...
        if self.path != None and len(self.path) > 1 and self.path.contains(x, y):
    ...
    def insert_point(self, x, y):
        ...
        We first need to check if the mouse location is on the path. Inserting
        point is time intensive and experimental.
        """
    ...
    def update(self):
        ...
        Alters the path by allowing the user to add new points, drag point
        handles and move their location. Updates are automatically stored as SVG
        ...
        if _ctx._ns["mousedown"] and not self.freehand:
            ...
            if (
                ...
                dx += self.btn_r * 2 + 2
                if self.edit == len(self._points) - 1 and self.overlap(
                    dx,
                    dy,
                    x,
                    y,
            ...
            # enter edit mode.
            if not self.drag_point and not self.drag_handle1 and not self.drag_handle2:
                self.editing = False
                ...
                for i in indices:
                    ...
                    if (
                        ...
                        if self.edit == i + 1 and self.overlap(
                            self._points[i + 1].ctrl1.x,
                            self._points[i + 1].ctrl1.y,
                            x,
                            y,
                        ):
                            continue
            ...
            if not self.editing:
                if self.edit != None:
                    pt = self._points[self.edit]
                    if self.overlap(pt.ctrl1.x, pt.ctrl1.y, x, y) or self.overlap(
                        pt.ctrl2.x,
                        pt.ctrl2.y,
                        x,
                        y,
            ...
            if self.edit == None:
                if self.new == None:
                    ...
                    if self.moveto == True or len(self._points) == 0:
                        cmd = MOVETO
                        self.moveto = None
            ...
            # Edit mode
                ...
                if (
                    self.overlap(pt.ctrl1.x, pt.ctrl1.y, x, y)
                    and pt.cmd == CURVETO
                    and not self.drag_point
                    and not self.drag_handle2
                ):
                ...
                if self.drag_handle1 == True:
                    pt.ctrl1 = Point(x, y)
                    if self.edit > 0 and self.last_key != "x":
                ...
                if self.drag_handle2 == True:
                    pt.ctrl2 = Point(x, y)
                    if self.edit < len(self._points) - 1 and self.last_key != "x":
                        next = self._points[self.edit + 1]
    ...
    def draw(self):
        ...
        if len(self._points) > 0:
            ...
            for i in range(len(self._points)):
                ...
                if (
                    ((i == self.edit and self.new == None) or pt == self.new)
                    and pt.cmd == CURVETO
                    and not pt.freehand
                ):
                ...
                if pt == self.new and not pt.freehand:
                ...
                # highlight it.
                elif self.overlap(x, y, pt.x, pt.y) and not pt.freehand:
            ...
            elif self.edit == None and self.new == None and self.moveto != None:
                _ctx.stroke(self.new_color)
                _ctx.nofill()
                _ctx.oval(x - r * 0.8, y - r * 0.8, r * 1.6, r * 1.6)
